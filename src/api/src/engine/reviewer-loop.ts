...
function buildSystemPreamble(): string {
  return [
    ...
    '',
    '🛑 You are READ-ONLY. Do NOT write, edit, create, or delete source files. Do',
    'NOT commit, push, deploy, or run destructive commands. You may run existing',
    'non-destructive build, type-check, lint, and test commands; Liliput sanitises',
    'the worktree after your turn so generated verification output is removed.',
    '',
    ...
    '  - generated dependencies, caches, build output, secrets, environment',
    '    files, or unrelated changes included in the diff',
    '  - changed behavior without executable coverage, or a relevant existing',
    '    build/test command that fails',
    '',
    ...
    '',
    'For workspace reviews, inspect the actual diff and run the smallest relevant',
    'existing verification command when practical. A failed command or inability',
    'to verify core behavior is FEEDBACK, not NO-FEEDBACK.',
  ].join('\n');
}
...
function buildCoderPrompt(ctx: ReviewContextCoder): string {
  ...
  return [
    ...
    'Inspect the diff and full affected files. Run the smallest relevant existing build,',
    'type-check, lint, or test command when practical. Report command failures, missing',
    'acceptance coverage, generated artifacts, secrets, and unrelated changes.',
    '',
    'Reply with `NO-FEEDBACK` or `FEEDBACK\\n- …` per the format above.',
  ]
    .filter((s) => s !== '')
