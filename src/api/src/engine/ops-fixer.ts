...
function buildPrompt(opts: OpsFixerOptions): string {
  ...
  const guardrails = [
    ...
    '## Tool wishes',
    '',
    '  - If during your work you wish you had a CLI tool that is NOT installed in this',
    '    environment (e.g. `jq`, `yq`, `ripgrep`, `helm`, `terraform`, `psql`), emit',
    '    a single line in your reply of the form:',
    '        TOOL-WISH: <tool> — <one-line reason>',
    '    The line must start with `TOOL-WISH:` (case-insensitive) and contain the tool',
    '    name on its own. Liliput captures these so the operator can bake popular',
    '    requests into the next runtime image. Emitting wishes does NOT affect this',
    '    fix attempt — keep working with what you have.',
    '',
    '## Soft guidance',
    '',
