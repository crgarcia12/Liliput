...
function buildPrompt(title: string, description: string, repoBlob: string | null): string {
  const sections: string[] = [
    ...
    '',
    'Liliput users intentionally provide minimal prompts and expect a ready-to-use,',
    'high-quality software result. Expand sparse requests with conventional,',
    'production-ready defaults for the product category: complete critical user',
    'flows, responsive and accessible UX, safe data handling, explicit errors,',
    'configuration, observability, automated tests, and deployability when relevant.',
    'Do not invent business-specific features or broaden the product beyond the',
    'stated goal. Record inferred choices as assumptions. Prefer safe, reversible,',
    'configurable defaults when a detail is missing.',
    '',
    'The spec MUST contain these sections in this order:',
    '1. `# Specification: <title>`',
    '2. `## Overview` — 2–4 sentences summarising the goal',
    '3. `## Assumptions and Defaults` — bullets for every inferred product or technical choice',
    '4. `## Requirements` — numbered functional + non-functional requirements',
    '5. `## Acceptance Criteria` — checkbox list (`- [ ]`) covering observable outcomes',
    '6. `## Technical Approach` — ordered steps an engineer would follow',
    '7. `## Out of Scope` — bullets listing what is explicitly NOT included',
    '8. `## Acceptance Scenarios (Gherkin)` — a fenced ```gherkin code block',
