...
export function buildDeployContract(ctx: DeployContractContext): string {
  ...
  return [
    ...
    `7. **Base images: prefer \`mcr.microsoft.com/...\` over Docker Hub.** \`az acr build\` runs on shared agents that anonymously pull public base images, and Docker Hub can rate-limit them. For Node, choose an Azure Linux \`nodejs:<major>\` tag that satisfies the app's \`engines.node\` and build-tool requirements; do not force Node 20 when the project requires Node 22 or 24. For Python, \`mcr.microsoft.com/azurelinux/base/python:3.12\` is a common default. Verify the runtime version during the build.`,
    '',
    '## How to verify',
    '',
    ...
    `- \`BASE_PATH\` — the public browser prefix (= \`${pathPrefix}\`). Use it for generated asset URLs and browser API calls, not for server route mounting.`,
    `- \`NEXT_PUBLIC_BASE_PATH\` — the same public prefix (= \`${pathPrefix}\`) is present in the running pod. Frameworks that inline \`NEXT_PUBLIC_*\` during image build must also bake this value into the bundle; runtime injection alone does not rewrite static assets.`,
